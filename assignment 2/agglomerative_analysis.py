...
# 데이터 로딩 및 전처리
print("=== Agglomerative Clustering 분석 ===")
print("데이터 로딩 중...")
...
plt.tight_layout()
plt.savefig('agglomerative_analysis.png', dpi=300, bbox_inches='tight')
plt.show()

# 2. Ward Linkage 상세 분석
print(f"\n2. Ward Linkage 상세 분석...")

ward_linkage = linkage(X_pca, method='ward')
...
# 2.2 Silhouette Score와 Inertia 함께 분석
plt.subplot(1, 2, 2)
...
for n_clusters in cluster_range:
    ...
    # sklearn AgglomerativeClustering으로 inertia 계산
    agg_model = AgglomerativeClustering(n_clusters=n_clusters, linkage='ward')
    agg_clusters = agg_model.fit_predict(X_pca)
    
    # 수동으로 inertia 계산 (클러스터 내 제곱합)
    inertia = 0
    for cluster_id in np.unique(agg_clusters):
        cluster_points = X_pca[agg_clusters == cluster_id]
...
plt.title('Ward Clustering: Silhouette Score vs Inertia', fontsize=16)
plt.tight_layout()
plt.savefig('ward_detailed_analysis.png', dpi=300, bbox_inches='tight')
plt.show()
...
# 최적 클러스터 수로 클러스터링
optimal_clusters = fcluster(ward_linkage, best_n_clusters, criterion='maxclust')
...
plt.title(f'Ward Clustering (n={best_n_clusters})\nSilhouette Score: {best_score:.4f}')
plt.xlabel('PC1')
plt.ylabel('PC2')
...
plt.tight_layout()
plt.savefig('optimal_clustering_results.png', dpi=300, bbox_inches='tight')
plt.show()
...
print(f"\n=== 분석 완료 ===")
print("생성된 파일들:")
print("- agglomerative_analysis.png: 4가지 linkage 방법의 dendrogram + silhouette 분석")
print("- ward_detailed_analysis.png: Ward 방법의 상세 분석 (silhouette + inertia)")
print("- optimal_clustering_results.png: 최적 클러스터링 결과") 