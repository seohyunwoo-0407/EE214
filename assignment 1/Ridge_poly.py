...
def fit_polynomial_ridge(x,y,degree, alpha): # fit polynomial model with ridge regularization
    poly=PolynomialFeatures(degree=degree)
    X_poly=poly.fit_transform(x)
...
for alpha in alphas:
    ...
    for degree in degrees:
        ...
        for fold, (train_idx, val_idx) in enumerate(kf.split(X_train)):
            ...
            model, poly=fit_polynomial_ridge(X_train_fold,y_train_fold,degree, alpha)
...
plt.grid(True)
plt.yscale('log')
plt.savefig('Ridge_cross_validation_alpha.png')
plt.show()

# Ridge 모델의 최적값 찾기
best_ridge_alpha = None
best_ridge_degree = None
...
print(f"Degree: {best_ridge_degree}")
print(f"Validation MSE: {best_ridge_mse:.4f}")
