#Train linear classifier(perceptron) with mini-batch gradient descent
...
for i in range(max_iter):
    ...
    for j in range(n_batch  ):
        ...
        t_batch=T_train_shuffled[j*batch_size:(j+1)*batch_size]

        Pred_train = np.matmul(x_batch, ab) + c
        Y_train = 2 * ((Pred_train) >= 0) - 1

        loss=np.sum(Pred_train*(Y_train-t_batch))

        grad_a=np.sum(x_batch[:,0]*(Y_train-t_batch))
        grad_b=np.sum(x_batch[:,1]*(Y_train-t_batch))
        grad_c=np.sum(Y_train-t_batch)
...
plt.savefig('linear_classification/linear_classifier_mini-GD.png')
plt.show()
